...
function (S::Simulation)(
    ...
    # Set up callback for updating history stack
    ts = (S.t0 + τ.Δt) : τ.dt : S.tf
    affect! = icl_cbf_mbrl_gradient_affect!
    cb = PresetTimeCallback(ts, affect!)

    problem = ODEProblem(rhs!, vcat(x, Wc, Wa, θ̂, P.ϑ), S.tf, p)
    trajectory = solve(problem, callback=cb, tstops=ts)
...
"""
    icl_cbf_mbrl_gradient_affect!(integrator)

Stop integration and update history stack with current data.
"""
function icl_cbf_mbrl_gradient_affect!(int)
    # Pull out system dimensions
    n = int.p["Σ"].n
    L = int.p["ϕ"].L
    p = int.p["P"].p

    # Construct trajectory function
    x(t) = n == 1 ? int.sol(t, idxs=1) : int.sol(t, idxs=1:n)
    ...
    if stack_difference(int.p["τ"].stack, F) >= int.p["τ"].stack.ε
        # If so integrate other objects of interest and update stack
        Wa(t) = int.sol(t, idxs=(n + L + 1 : n + 2*L))
        θ̂(t) = p == 1 ? int.sol(t, idxs=(n + 2*L + 1)) : int.sol(t, idxs=(n + 2*L + 1 : n + 2*L + p))
        ϑ(t) = int.sol(t, idxs=(n + 2*L + p + 1))
        update_stack!(
            int.p["τ"].stack, 
            integrate_state_derivative(x,  int.t, int.t - int.p["τ"].Δt), 
            integrate_drift(x, int.t, int.t - int.p["τ"].Δt, int.p["Σ"]),
            F, 
            integrate_control(x, Wa, θ̂, ϑ, int.t, int.t - int.p["τ"].Δt, int.p["Σ"], int.p["k"])
            )
    end
